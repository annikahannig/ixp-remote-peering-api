...
def _query_modifiers(param_schema):
    ...
    if param_schema[1] == range:
        return param_schema[3:]
...
def whitelist_params(params, schema):
    ...
    for param, value in params.iteritems():
        ...
        # Automodifier
        s_type = schema[param][1] # Schema type
        q_type = _query_type(schema[param])
        if type(s_type) == list:
            modifier = 'in'
        elif s_type == range:
            modifier = schema[param][3]
        ...
        if modifier:
            q_param = "{}__{}".format(q_field, modifier)
        else:
            q_param = q_field
...
def filters_from_query_params(params, schema):
    ...
    Example:
        ...
        'names': ('name', [str], ),
        'name': ('name', str, 'icontains', 'contains'),
    ...
    Example:
        ...
        'date_start': ('created_at', range, datetime, 'gte')
